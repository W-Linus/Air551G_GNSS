...
void air551g_init(){
	HAL_Delay(3000);
	HAL_UART_Transmit(AIR551G_uart,Init_HotStartup,sizeof(Init_HotStartup),0xFFFF);
	HAL_Delay(500);
...
GNSS_info gnss_getLocation(GNSS_info gnssdata){
	sscanf((char*)rptBuffer+commaPos[2]+1,"%lf",&gnssdata.latitude);
	sscanf((char*)rptBuffer+commaPos[3]+1,"%c",&gnssdata.lat_dir);
	sscanf((char*)rptBuffer+commaPos[4]+1,"%lf",&gnssdata.longitude);
	sscanf((char*)rptBuffer+commaPos[5]+1,"%c",&gnssdata.long_dir);
	gnssdata.latitude=gnssdata.latitude/100.0;
	gnssdata.longitude=gnssdata.longitude/100.0;
	return gnssdata;
}
